...
figure('color','w')
TITLE = 'Membrane Potential Response with Two 15 μA Stimuli';
subplot(2,1,1), plot(t_plot, v_plot, 'k', 'LineWidth', 2)
xlabel('Time (ms)'), ylabel('Membrane Potential (mV)')
title(TITLE), grid on

subplot(2,1,2), plot(t_plot, mhn_plot(1,:), 'r', ...
                     t_plot, mhn_plot(2,:), 'b', ...
                     t_plot, mhn_plot(3,:), 'g', 'LineWidth', 2)
xlabel('Time (ms)'), ylabel('Channel State (%)')
legend('m: Na⁺ activation', 'h: Na⁺ inactivation', 'n: K⁺ activation')
title('Ion Channel Gating Dynamics'), grid on
